...
from .batch import CsvBatch, JsonBatch
from .error import SelectdbException
from .http_builder import HttpPostBuilder, HttpPutBuilder, requests
...
class SelectDBStageLoad:
    ...
    def load_list(self, rows: list[list], db, table, columns: list = None):
        ...
        if self.copy_into_props["file.type"] == "csv":
            ...
            for i, row in enumerate(rows):
                batch.add_line(row)
                if (
                    batch.get_capacity() > self.file_split_size
                    or len(batch.batch_data) >= self.row_batch_size
                ):
            ...
            try:
                for i, row in enumerate(rows):
                    ...
                    if (
                        json_batch.get_capacity() >= self.file_split_size
                        or len(json_batch.batch_data) >= self.row_batch_size
                    ):
                        upload_file_name = file_name % file_num
                        file_num += 1
                        address = self.get_upload_address(upload_file_name)
    ...
    def execute_copy(self, file_name, db, table):
        ...
        copy_sql_builder = CopySQLBuilder(
            self.copy_into_props, f"{db}.{table}", file_name
        )
        ...
        post_builder.set_url(f"{self.commit_url_str}").base_auth(
            self.user, self.passwd
        ).set_entity(json.dumps(params))
        ...
        with requests.Session() as session:
            ...
            if status_code != 200:
                print(
                    f"commit failed with status {status_code} {self.host_port}, reason {reason_phrase}"
                )
                ...
                if success:
                    print(
                        f"commit success cost {int(time.time() * 1000) - start}ms, response is {load_result}"
                    )
                else:
    ...
    def handle_commit_response(self, load_result):
        ...
        if base_response["code"] == SelectDBStageLoad.SUCCESS:
            ...
            if base_response["code"] == SelectDBStageLoad.FAIL:
                ...
                if result["state"] != "FINISHED" and not self.is_committed(
                    result["msg"]
                ):
    ...
    def upload_file(self, address, value, file_name):
        ...
        put_builder.set_url(address).add_common_header().set_entity(
            value.encode("utf-8")
        )
    ...
    def get_upload_address(self, file_name, prepared_reques=None):
        ...
        put_builder.set_url(self.load_url_str).add_file_name(
            file_name
        ).add_common_header().set_empty_entity().base_auth(self.user, self.passwd)
