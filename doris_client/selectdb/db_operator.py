...
class Table:
    def __init__(
        ...
        self.table_model_key = (
            ",".join(table_model_key)
            if table_model_key is not None
            else self.gen_key_default()
        )
        self.distributed_hash_key = (
            ",".join(distributed_hash_key)
            if distributed_hash_key is not None
            else self.gen_key_default()
        )
    ...
    def gen_distribute_info(self):
        return (
            f"DISTRIBUTED BY HASH ({self.distributed_hash_key}) BUCKETS {self.buckets}"
        )
